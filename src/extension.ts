...
import { FloatingTerminalManager } from './floatingTerminalManager';

let floatingTerminalManager: FloatingTerminalManager;

export function activate(context: vscode.ExtensionContext) {
    console.log('Floating Terminal extension is now active!');
    
    floatingTerminalManager = new FloatingTerminalManager(context);
    ...
    if (!hasShownWelcome) {
        vscode.window.showInformationMessage(
            'Floating Terminal is ready! Press Ctrl+` (Cmd+` on Mac) to toggle your floating terminal.',
            'Show Keymaps'
        ).then(selection => {
